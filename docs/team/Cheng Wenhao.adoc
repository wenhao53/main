...
changes after is reflected intuitively as the background colour of the PersonCard to reduce the cluttering of information displayed on the card.
...
** Highlights: the background colour of the PersonCard is changed immediately upon any changes to the BMI classification with each edit command. Listing of clients with the same BMI classification is intuitive since all clients with the same BMI classification have the same background colour for their card.
...
* *Minor enhancement*: edited the case-sensitivity of the `add` command to ensure that clients with the exact same alphabetical name (may be different case) and same attributes will not be added as a duplicate.
