= Personal Trainer Pro (PTP) - User Guide
:toc:
:toc-title:
...
Selects the person identified by the index number used in the last person listing and displays the person card that can be viewed by hovering mouse over the card. +
Format: `select INDEX`
Alias: `s`

****
* Selects the person and loads the person card at the specified `INDEX`.
* The index refers to the index number shown in the most recent listing.
* The index *must be a positive integer* `1, 2, 3, ...`
****
...
=== Weight Logging for clients `[coming in v2.0]`

Users will be able to view the history of weight changes for each client in a graphical and list format.
